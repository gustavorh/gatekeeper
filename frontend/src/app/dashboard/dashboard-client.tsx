...
import { useCurrentShift } from "@/lib/hooks/use-shifts";

interface DashboardClientProps {
  fallback?: Record<string, unknown>;
  initialRecentActivities?: any[];
  initialCurrentShift?: any;
}

function DashboardContent({
  initialRecentActivities = [],
  initialCurrentShift = null,
}: Pick<DashboardClientProps, "initialRecentActivities" | "initialCurrentShift">) {
  ...
  const { data: currentShiftData, mutate: mutateShift } = useCurrentShift<any>();
  const currentShift = currentShiftData ?? initialCurrentShift;
  const loadingShift = currentShiftData === undefined && initialCurrentShift === null;

  const isClockedIn = currentShift?.status === "active" || currentShift?.status === "on_lunch";
  const isOnLunch = currentShift?.status === "on_lunch";
  ...
  // Helper function to format delay time in "00h 00m 00s" format
  ...
  const refreshActivities = async () => {
    try {
      ...
      if (Array.isArray(shifts) && shifts.length > 0) {
        ...
        setRecentActivities([sortedShifts[0]]);
      } else {
        setRecentActivities([]);
      }
    } catch {
      // silently keep previous activities
    } finally {
  ...
  const handleStartLunch = async () => {
    try {
      await apiClient.post("/shifts/start-lunch", {});
      await mutateShift();
  ...
  const handleResumeShift = async () => {
    try {
      await apiClient.post("/shifts/resume-shift", {});
      await mutateShift();
